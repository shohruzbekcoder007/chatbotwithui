...
class InvestAsosiy4ToolInput(BaseModel):
    so_rov: str = Field(description="4-invest asosiy hisoboti ma'lumotlarini qidirish uchun so'rov.")

class InvestAsosiy4Tool(BaseTool):
    """4-invest asosiy hisoboti ma'lumotlarini qidirish va tahlil qilish uchun vosita."""
    
    name: str = "invest_asosiy4_tool"
    description: str = "4-invest asosiy hisoboti ma'lumotlarini qidirish va tahlil qilish uchun vosita."
    args_schema: Type[BaseModel] = InvestAsosiy4ToolInput

    embedding_model: Optional['CustomEmbeddingFunction'] = Field(default=None)
    invest_asosiy4_data: Dict = Field(default_factory=dict)
    ...
    def __init__(self, transport4_file_path: str, use_embeddings: bool = True, embedding_model=None):
        super().__init__()
        self.invest_asosiy4_data = self._load_transport4_data(transport4_file_path)
    ...
    def _load_transport4_data(self, transport4_file_path: str) -> Dict:
        """4-invest asosiy hisoboti ma'lumotlarini yuklash."""
        try:
            with open(transport4_file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            logging.info("4-invest asosiy hisoboti ma'lumotlari muvaffaqiyatli yuklandi")
            return data
    ...
    def _prepare_embedding_data(self):
        ...
        for item in self.invest_asosiy4_data:
            if 'report_form' in item:
                report_form = item['report_form']
                text = f"4-invest asosiy hisoboti: {report_form.get('name', '')}. {report_form.get('submitters', '')}. {report_form.get('structure', '')}. {report_form.get('indicators', '')}"

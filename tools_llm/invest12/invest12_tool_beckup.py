...
class Invest12ToolInput(BaseModel):
    so_rov: str = Field(description="12 invest ma'lumotlarini qidirish uchun so'rov.")

class Invest12Tool(BaseTool):
    """12 invest ma'lumotlarini qidirish va tahlil qilish uchun vosita."""
    
    name: str = "invest12_tool"
    description: str = "12 invest ma'lumotlarini qidirish va tahlil qilish uchun vosita."
    ...
    def __init__(self, invest12_file_path: str, use_embeddings: bool = True, embedding_model=None):
        super().__init__()
        self.invest12_data = self._load_invest12_data(invest12_file_path)
        self.use_embeddings = use_embeddings
        self.embedding_model = embedding_model
        if embedding_model is not None:
            self._prepare_embedding_data()

    def _load_invest12_data(self, invest12_file_path: str) -> List[Dict]:
        """Invest12 ma'lumotlarini yuklash."""
        with open(invest12_file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        logging.info("Invest12 ma'lumotlari muvaffaqiyatli yuklandi")
        
        # Agar data lug'at bo'lsa, uni ro'yxat ichiga o'rab qaytarish
    ...
    def _prepare_embedding_data(self):
        ...
        if isinstance(self.invest12_data, dict):
            items = [self.invest12_data]
        else:
            items = self.invest12_data
        ...
        for item in items:
            if 'report_form' in item:
                ...
                text = f"Invest12: {report_form.get('name', '')}. {report_form.get('submitters', '')}. {report_form.get('structure', '')}. {report_form.get('indicators', '')}"
            ...
            if 'sections' in item:
                ...
                if section.get('section_type', '') == 'statistic' and 'rows' in section:
                    for row in section['rows']:
                        ...
                        qator_match = False
                        if row_code:
                            qator_match = row_code == qator_part
                            if qator_match:
                                found_qator = True
                        ...
                        if 'columns' in row:
                            for column in row['columns']:
                                ...
                                if (qator_match and ustun_match) or \
                                   ...
                                   (ustun_match and not row_code) or \
            ...
            if section.get('section_type', '') == 'dynamic' and 'columns' in section:
                ...
                for column in section['columns']:
                    ...
                    if ustun_num:
                        ...
                        ustun_match = ustun_num.upper() == ustun_part.upper()
                        if ustun_match:
                            found_ustun = True
                    ...
                    if (so_rov.lower() in ustun_tavsifi.lower()) or \
                       (ustun_match) or \
                       (bob_part and ustun_num and bob_part in bob and ustun_num.upper() == ustun_part.upper()) or \
                       (bob_part and not ustun_num and bob_part in bob):
    ...
    def _run(self, so_rov: str) -> Dict:
        ...
        # Semantik qidiruv har doim ishga tushiriladi
        if self.embedding_model is not None:
            ...
            if semantic_results:
                ...
                logging.info(f"Semantik qidiruv natijasida {len(semantic_results)} ta natija topildi, {added_count} ta qo'shildi")
        
        # Agar natijalar bo'sh bo'lsa va embedding modeli mavjud bo'lsa, semantik qidiruvni majburiy ishlatish
        if not text_results and self.embedding_model is not None:
